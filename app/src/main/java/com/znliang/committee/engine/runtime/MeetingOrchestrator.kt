    ...
    suspend fun preMeetingIntelSearch(subject: String) {
        try {
            ...
            if (searchResults.isNotEmpty()) {
                ...
                ctx.updateBoard { it.copy(preGatheredInfo = info) }
...
- 互动质量(INTERACT)：是否回应他人、是否有建设性交锋"""
    ...
    suspend fun runLoop() {
        ...
        while (!ctx.boardValue.finished) {
            ...
            val minMsgThreshold = ctx.agents.size * 2  // 每个 agent 平均至少发言 2 次
            val minRoundThreshold = ctx.debateRounds   // mandate-driven minimum rounds
